		...
		select
			...
			r.OrderType,
			r.DocumentStatus,
			JSONQuery.[rep_first_name] + ' ' + JSONQuery.[rep_last_name] as [Rep. Name],
			JSONQuery.[rep_phone] as [Rep. Phone],
			JSONQuery.[rep_email] as [Rep. Email],
			...
			JSONQuery.[bill_to_city]		as [ECAT_Incoming_BillToCity],
			ECAT_Return.BillToCity			as [ECAT_Outbound_BillToCity],
			JSONQuery.[bill_to_state]		as [ECAT_Incoming_BillToState],
			ECAT_Return.BillToState			as [ECAT_Outbound_BillToState],
			...
			JSONQuery.[ship_to_city]		as [ECAT_Incoming_ShipToCity],																					
			ECAT_Return.ShipToCity			as [ECAT_Outbound_ShipToCity],																					
			JSONQuery.[ship_to_state]		as [ECAT_Incoming_ShipToState],																					
			ECAT_Return.ShipToState			as [ECAT_Outbound_ShipToState],																					
			JSONQuery.[ship_to_postal_code]	as [ECAT_Incoming_ShipToPostalCode],																											
			ECAT_Return.ShipToPostCode		as [ECAT_Outbound_ShipToPostalCode]
		into #temp_jkp_data
		from SysproDocument.ESI.Stage_SalesOrder_Raw r
			cross apply openjson(r.RawDocumentText)
			with (
					[rep_first_name]		    varchar(40) '$.rep_first_name',
					[rep_last_name]				varchar(40) '$.rep_last_name',
					[rep_phone]					varchar(40) '$.rep_phone',
					[rep_email]					varchar(40)	'$.rep_email',
...
select
	...
	STUFF(
		(
			SELECT ', ' + b.EcatOrderNumber from #temp_jkp_data b
			where b.[Rep. Name] = a.[Rep. Name]
				and b.[Rep. Email] = a.[Rep. Email]
				and b.[Rep. Phone] = a.[Rep. Phone]
			for xml path('')
		 )	
		,1,2,'') as [Quotes]
from #temp_jkp_data a
group by [Rep. Name], [Rep. Email], [Rep. Phone]

drop table #temp_jkp_data