...
if exists(select 1 from [PRODUCT_INFO].[sys].[tables] where [tables].[name] ='SalesOrderLineDelete_Ref')
begin
...
if exists(select 1 from [PRODUCT_INFO].[sys].[tables] where [tables].[name] ='SalesOrderLineDeleteExport_Archive')
begin
	...
	from [SugarCrm].[JobOptions] as ops
		inner join (
						...
						from (
							...
							union
							select 'Invoice Line Items', count(IL.DetailLine) from [SugarCrm].[tvf_BuildInvoiceLineDataset]() as IL
							union
							select '', count(OLD.[SalesOrderInitLine]) from [SugarCrm].[tvf_BuildSalesOrderLineDeleteDataset]() as OLD ) as [Counts]
						where [Counts].[CNT] > 0
