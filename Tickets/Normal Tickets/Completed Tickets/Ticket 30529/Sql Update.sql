Declare @CopyBranch varchar(5) = '315'
Declare @NewBranch varchar(5) = '316'
 
INSERT INTO SysproDocument.WCS.Ref_GlCode (BranchId, BankId, SPostingGlCode, StorePostingGlCode, TransferReference, StoreReference, TransferNarration, StoreNarration)
SELECT @NewBranch as BranchId
	, REPLACE(BankId, @CopyBranch, @NewBranch) as BankId
	, REPLACE(SPostingGlCode, @CopyBranch, @NewBranch) as SPostingGlCode
	, REPLACE(StorePostingGlCode, @CopyBranch, @NewBranch) as StorePostingGlCode
	, TransferReference, StoreReference, TransferNarration, StoreNarration
FROM SysproDocument.WCS.Ref_GlCode 
WHERE BranchId = @CopyBranch

select
	*
from WCS.Ref_GlCode
where BranchId = @NewBranch
