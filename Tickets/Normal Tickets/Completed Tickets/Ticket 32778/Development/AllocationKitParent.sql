...
WITH ComponentDetail AS (
			...
			FROM SysproCompany100.dbo.SorMaster SM
				INNER JOIN SysproCompany100.dbo.SorDetail SD ON SM.SalesOrder = SD.SalesOrder 
															AND SD.LineType = 1
				INNER JOIN SysproCompany100.dbo.[CusSorDetailMerch+] SDP ON SDP.SalesOrder = SD.SalesOrder 
																		AND SD.SalesOrderInitLine = SDP.SalesOrderInitLine 
																		AND SDP.InvoiceNumber = ''
...
,ParentDetail AS (
					SELECT 
						...
						,SDP.AllocationDate
						,MAX(CD.ComponentDate) AS ParentDate
					FROM SysproCompany100.dbo.SorDetail SD
						INNER JOIN SysproCompany100.dbo.[CusSorDetailMerch+] SDP ON SD.SalesOrder = SDP.SalesOrder 
																				AND SD.SalesOrderInitLine = SDP.SalesOrderInitLine 
																				AND SDP.InvoiceNumber = ''
						INNER JOIN ComponentDetail CD ON CD.SalesOrder = SD.SalesOrder 
													 AND CD.ParentLine = SD.SalesOrderLine
...
SELECT 
	SalesOrder,
	SalesOrderInitLine,
	ParentDate as [AllocationDate]
FROM ParentDetail
where ParentDate <> @Placeholder